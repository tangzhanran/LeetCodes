//	454. 4Sum II
//
//	Given four lists A, B, C, D of integer values, compute how many tuples(i, j, k, l) there are such that A[i] + B[j] + C[k] + D[l] is zero.
//
//	To make problem a bit easier, all A, B, C, D have same length of N where 0 ¡Ü N ¡Ü 500. All integers are in the range of - 228 to 228 - 1 and the result is guaranteed to be at most 231 - 1.
//
//	Example:
//
//	Input:
//	A = [1, 2]
//	B = [-2, -1]
//	C = [-1, 2]
//	D = [0, 2]
//
//	Output :
//	2
//
//	Explanation :
//	The two tuples are :
//	1. (0, 0, 0, 1)->A[0] + B[0] + C[0] + D[1] = 1 + (-2) + (-1) + 2 = 0
//	2. (1, 1, 0, 0)->A[1] + B[1] + C[0] + D[0] = 2 + (-1) + (-1) + 0 = 0
//
//	Written by Zhanran Tang @ 8/23/2017
//
//	Ideas: Calculate the sum of A&B, store the sum times in hashmap. 
//		Calculate the sum of C&D, find the negative value of this sum in hash and add times to final result.
//	Complexity: O(n^2) in time and space.

#include "Header.h"

using namespace std;

int fourSumCount(vector<int>& A, vector<int>& B, vector<int>& C, vector<int>& D)
{
	unordered_map<int, int> sumtimes; //value,times
	int res = 0;
	for (int i = 0; i<A.size(); i++)
	{
		for (int j = 0; j<B.size(); j++)
		{
			int sum = A[i] + B[j];
			if (sumtimes.find(sum) == sumtimes.end())
				sumtimes[sum] = 0;
			sumtimes[sum]++;
		}
	}
	for (int i = 0; i<C.size(); i++)
	{
		for (int j = 0; j<D.size(); j++)
		{
			int sum = -(C[i] + D[j]);
			if (sumtimes.find(sum) != sumtimes.end())
				res += sumtimes[sum];
		}
	}
	return res;
}

int main()
{
	vector<int> A = { 1, 2 };
	vector<int> B = { -2, -1 };
	vector<int> C = { -1, 2 };
	vector<int> D = { 0, 2 };
	cout << fourSumCount(A, B, C, D) << endl;
	system("pause");
	return 0;
}